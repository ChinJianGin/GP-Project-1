...
//	}
//	// 讓 boy 1.5 秒完成一次跳耀，以 sinf 函式負責高度的基礎
//	// 也就是 1.5 秒從 0 到 PI
//	if ( _bBoyJump )
//	{
//		_fjtime += dt;
//		float dh = sinf(_fjtime*M_PI/1.0f);
//		cocos2d::Point pt = _boyRoot->getPosition();
//		pt.y = dh * 150 + _boypt.y;
//		_boyRoot->setPosition(pt);	
//		if (_fjtime > 1.0f) { // 停止跳耀
//			_bBoyJump = false;
//			_fjtime = 0;
//			// 重設 boy Y軸高度, 避免多跳幾次後產生誤差
//			_boyRoot->setPosition(_boyRoot->getPosition().x ,_boypt.y);
//			_boyAction->setTimeSpeed(1.0f); // 一倍速
//		}
//	}
//}
...
//		// 儲存觸控點座標與對應的 ID
//
//		if ( _runbtn->touchesBegin(touchLoc) )  { // 這個 touch 點，點在 runbtn 上
//			_irunid = touchId; _bBoyRun = true;
//		}
//		else if ( _jumpbtn->touchesBegin(touchLoc) ) {// 目前不是在跳耀狀態,按下即跳
//			_ijumpid = touchId;
//			if (_bBoyJump == false) {
//				_bBoyJump = true;
//				_boypt = _boyRoot->getPosition(); // 取得目前男孩的位置
//				_boyAction->setTimeSpeed(0.35f); // 一倍速
//			}
//		}
//		else {
...
//			_irunid = -1;
//		}
//		else if ( _ijumpid == touchId ) { // 這個 touch 點之前是點在 runbtn 上
